...
from rab import basis


CPP_EXECUTABLE = f"clebschgordan.out"
...
IRREPS = lambda d, n: [[2 * l] + [l] * (d - 2) + [0] for l in range(0, n + 1)]
DIM_IRREPS = lambda d, n: [int((d + 2 * l - 1) / (d -1) * binom(d + l - 2, l) ** 2) for l in range(0, n + 1)]
...
0'''
    ...
    print(os.path.isfile(CPP_EXECUTABLE))
...
def build_projector_noninteracting(d, n, irrep_index):

    dF = int(binom(d + n - 1 , n))
    sym_basis = np.eye(dF)
    conj_basis = np.diag(compute_minus_positions(CONJ_SYM_IRREP(d, n)))[::-1]

    irrep3, dimirrep3 = IRREPS(d,n)[irrep_index], DIM_IRREPS(d,n)[irrep_index]
        
    with open(f"clebsch-gordan-coefficients/{d}modes_{n}particles/cgc_{'-'.join(map(str, irrep3))}.txt", "r") as file:
        cgc_table = np.loadtxt(file, skiprows=2)

    b3_basis = np.zeros((dimirrep3, dF ** 2))
    for b1, b2, b3, cgc in cgc_table:
        b3_basis[int(b3)-1] +=  cgc * np.kron(sym_basis[int(b1)-1],conj_basis[int(b2)-1])

    p = np.zeros((dF ** 2, dF ** 2))
    for k in range(dimirrep3):
        p += np.outer(b3_basis[k], b3_basis[k])
    
    return p

def build_projector_interacting(d, n):
    dF = int(binom(d + n - 1 , n))
    # basis vectors of H.
    b = basis.standard(dF)
    # |0>\otimes|0>, |1>\otimes|1>, ... |dF>\otimes|dF>
    b_ii  = np.einsum('nk,nl->nkl', b, b).reshape(dF, -1)
    # Create the maximally mixed density matrix, 1 minus this gives the nontrivial projector.
    p_trivial = np.sum(np.einsum('ij, kl -> ikjl', b_ii, b_ii), axis=(0,1)) / dF
    return np.eye(dF ** 2) - p_trivial
